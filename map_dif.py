...
def get_dif_filename(path, input, anat = None, behav = None, func = None, behav_input = None, smooth = None, sigma = None, strategy = None, 
    ...
    base = "{}/networks/_difference/{}_{}_".format(path, input, level)
...
def map_dif(path, input, anat = None, behav = None, func = None, level = None, behav_input = None, strategy = None, smooth = None, 
    ...
    if not os.path.exists("{}/networks/_difference".format(path)):
        os.mkdir("{}/networks/_difference".format(path))
    ...
    for key, dict in data.iteritems():
        if dict['MNI_COORDINATES']:
            ...
            # Load matrices with connections that appear in abstracts but not results
            elif input == "abstracts-results":
                for edge, weight in abs_anat_mat.iteritems():
                    if weight > 0:
                        if res_anat_mat[edge] == 0:
                            anat_mat[edge] += 1
                for edge, weight in abs_behav_mat.iteritems():
                    if weight > 0:
                        if res_behav_mat[edge] == 0:
                            behav_mat[edge] += 1
                for edge, weight in abs_func_mat.iteritems():
                    if weight > 0:
                        if res_func_mat[edge] == 0:
                            func_mat[edge] += 1
...
# map_dif(path, "results-abstracts", anat = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
# map_dif(path, "results-abstracts", behav = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
# map_dif(path, "results-abstracts", func = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
map_dif(path, "abstracts-results", anat = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
map_dif(path, "abstracts-results", behav = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
map_dif(path, "abstracts-results", func = True, level = "studies", window = "sentences", lemmas = True, neighbors = 100)
