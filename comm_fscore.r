...
# Set path for plots
plot.path <- "/Users/ehbeam/Dropbox/Stanford/Research/Projects/Psychiatlas/program/cogneuro/plots/fscores/"
...
plot.f.coords <- function(networks, file.name, type) {
  
  # Initialize variables for computing f-score
  sigmas <- c()
  modularities <- c()
  ...
  f.abs.bags <- 2 * modularities * alignments.abs.bags / (modularities + alignments.abs.bags)
  f.tex.bags <- 2 * modularities * alignments.tex.bags / (modularities + alignments.tex.bags)
  f.abs.wind <- 2 * modularities * alignments.abs.wind / (modularities + alignments.abs.wind)
  f.tex.wind <- 2 * modularities * alignments.tex.wind / (modularities + alignments.tex.wind)
  
  # Build data frame
  data <- data.frame(c(sigmas, sigmas, sigmas, sigmas), c(f.abs.bags, f.tex.bags, f.abs.wind, f.tex.wind), 
  ...
  colnames(data) <- c("sigma", "f", "words", "window")
  print(networks)
  print(data)

  # Plot scatter
  plot <- ggplot(data, aes(y = f, x = sigma)) +
    theme_classic() +
    geom_point(aes(color = words, shape = window), size = 1.5, stroke = 1.1) +
    scale_color_manual(values = adjustcolor(palette[2:3], 0.75)) +
    scale_shape_manual(values = c(16, 1)) +
    scale_y_continuous(limits = c(-0.001, 0.15), breaks = c(0.025, 0.075, 0.125)) +
...
p1 <- plot.f.coords(networks.wta.experiments.anat, "coords&words_exp_wta_anat", "anat")
p2 <- plot.f.coords(networks.wta.experiments.func.abstracts, "coords&words_exp_wta_func_abs", "func")
p3 <- plot.f.coords(networks.wta.experiments.func.texts, "coords&words_exp_wta_func_tex", "func")
...
p4 <- plot.f.coords(networks.pwta.experiments.anat, "coords&words_exp_pwta_anat", "anat")
p5 <- plot.f.coords(networks.pwta.experiments.func.abstracts, "coords&words_exp_pwta_func_abs", "func")
p6 <- plot.f.coords(networks.pwta.experiments.func.texts, "coords&words_exp_pwta_func_tex", "func")
...
p7 <- plot.f.coords(networks.p.experiments.anat, "coords&words_exp_prob_anat", "anat")
p8 <- plot.f.coords(networks.p.experiments.func.abstracts, "coords&words_exp_prob_func_abs", "func")
p9 <- plot.f.coords(networks.p.experiments.func.texts, "coords&words_exp_prob_func_tex", "func")
...
p10 <- plot.f.coords(networks.wta.studies.anat, "coords&words_stud_wta_anat", "anat")
p11 <- plot.f.coords(networks.wta.studies.func.abstracts, "coords&words_stud_wta_func_abs", "func")
p12 <- plot.f.coords(networks.wta.studies.func.texts, "coords&words_stud_wta_func_tex", "func")
...
p13 <- plot.f.coords(networks.pwta.studies.anat, "coords&words_stud_pwta_anat", "anat")
p14 <- plot.f.coords(networks.pwta.studies.func.abstracts, "coords&words_stud_pwta_func_abs", "func")
p15 <- plot.f.coords(networks.pwta.studies.func.texts, "coords&words_stud_pwta_func_tex", "func")
...
p16 <- plot.f.coords(networks.p.studies.anat, "coords&words_stud_prob_anat", "anat")
p17 <- plot.f.coords(networks.p.studies.func.abstracts, "coords&words_stud_prob_func_abs", "func")
p18 <- plot.f.coords(networks.p.studies.func.texts, "coords&words_stud_prob_func_tex", "func")
...
# Function to plot scatterplot of f-score by sigma, stratified by linking strategy, points labeled by type (anat or func)
plot.f.words <- function(networks, file.name) {
...
plot.f.words(networks.words.bags.anat, "words_bags_anat")
plot.f.words(networks.words.bags.behav, "words_bags_behav")
plot.f.words(networks.words.bags.func, "words_bags_func")
plot.f.words(networks.words.sentences.anat, "words_sentences_anat")
plot.f.words(networks.words.sentences.behav, "words_sentences_behav")
plot.f.words(networks.words.sentences.func, "words_sentences_func")
