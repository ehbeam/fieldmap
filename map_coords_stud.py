...
behav_inputs = ["results", "abstracts", "texts"]
lemmas = [True, None]
lev = "studies"
...
for strat in strategies:
	map_all.map(path, "coordinates", sigma = None, func = True, behav_input = "brainmap", level = lev, strategy = strat, smooth = False, force = True)
	# for inp in behav_inputs:
	# 	for lem in lemmas:
	# 		map_all.map(path, "coordinates", sigma = None, func = True, behav_input = inp, lemmas = lem, neighbors = 100, level = lev, strategy = strat, smooth = False, force = True)
	# 		for sig in sigmas:
	# 			map_all.map(path, "coordinates", sigma = sig, func = True, behav_input = "brainmap", level = lev, strategy = strat, smooth = True, force = True)
