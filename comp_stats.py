...
def comp(path, smooth = None):
    ...
    networks = [file.replace(".xml", "").replace("{}/networks/".format(path), "").replace("-", ".") for file in infiles if not "norm" in file and not "freqscaled" in file and not "thres" in file]
    ...
    for key, value in data.iteritems():
        ...
        if smooth:
            query = "{}/queries/gaussian/{}.txt".format(path, key)
        if not smooth:
            query = "{}/queries/raw/{}.txt".format(path, key)
