...
def load_prob_label(proc, smooth):
    ...
    if not smooth and "</b><br>" in proc:
        ...
        output = output[1] # Ignore the atlas, which is the first item of the list

        # Remove commas within anatomical labels so the output is splittable
        divisions = ["pars triangularis", "pars opercularis", "anterior division", "posterior division", "temporooccipital part", "superoir division", "superior division", "inferior division"]
        for division in divisions:
            output = output.replace(", " + division, " " + division)
...
def preproc(path, smooth = None, sigma = None, level = None, mask_path = None, dual_key = None):
    ...
    if smooth:
        ...
        if level == 'experiments':
            if not os.path.exists("{}/coordinates/experiments/struct_56/gaussian_{}mm".format(path, sigma)):
                os.makedirs("{}/coordinates/experiments/struct_56/gaussian_{}mm".format(path, sigma))
        elif level != 'experiments':
            if not os.path.exists("{}/coordinates/studies/struct_56/gaussian_{}mm".format(path, sigma)):
                os.makedirs("{}/coordinates/studies/struct_56/gaussian_{}mm".format(path, sigma))
    elif not smooth:
        log = "{}/logs/preproc_coords_{}_log.txt".format(path, level)
        if level == 'experiments':
            if not os.path.exists("{}/coordinates/experiments/struct_56/raw".format(path)):
                os.makedirs("{}/coordinates/experiments/struct_56/raw".format(path))
        elif level != 'experiments':
            if not os.path.exists("{}/coordinates/studies/struct_56/raw".format(path)):
                os.makedirs("{}/coordinates/studies/struct_56/raw".format(path))
    open(log, "w+").close()
    flog = open(log, "a")
    ...
    for key, dict in data.iteritems():
        ...
        query = "{}/coordinates/{}/struct_56/{}/{}.txt".format(path, level, sigma_dir, dict['KEY'])
        if dual_key == True:
            query = "{}/coordinates/{}/struct_56/{}/{} {}.txt".format(path, level, sigma_dir, dict['KEY'], dict['EXPERIMENT'].translate(None, string.punctuation).replace("  ", " "))
