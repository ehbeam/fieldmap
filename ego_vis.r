...
dir.create(paste(vis.path, "/ego", sep = ""))

# Load networks
networks <- grep("thres", list.files(pattern = ".*csv"), value = TRUE, invert = TRUE)
networks <- grep("studies_sentences_lemmas_anat", networks, value = TRUE)
...
cluster.nodes <- function(net, dir, g) {
  ...
  png(paste(dir, "/ego/", file_path_sans_ext(g), ".png", sep = ""), d, d)
...
lapply(networks, function(g, method) {
  ...
  # Drill down to single node
  net.vis <- graph.neighborhood(net, order = 1, nodes = which(V(net)$name == "frontal\npole"))
  
  # Export network plots
  cluster.nodes(net.vis[[1]], vis.path, g)
